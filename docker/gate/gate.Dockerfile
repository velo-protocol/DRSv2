...
## Set working directory
#WORKDIR /go/src/gitlab.com/velo-lab/core/gate
##Get artifact from buiber stage
#RUN mkdir -p app/migrations app/recovery_migrations
#COPY --from=builder /go/src/gitlab.com/velo-lab/core/gate/migrations/ gate/migrations/
#COPY --from=builder /go/src/gitlab.com/velo-lab/core/gate/recovery_migrations/ app/recovery_migrations/
#COPY --from=builder /usr/local/bin/wait-for /usr/local/bin/wait-for
#COPY --from=builder /go/src/gitlab.com/velo-lab/core/gate gate/
#COPY --from=builder /go/src/gitlab.com/velo-lab/core/gate/vendor vendor
## Set Docker's entry point commands
#CMD ./gate;